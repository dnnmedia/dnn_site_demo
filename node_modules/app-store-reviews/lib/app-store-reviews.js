...
AppStoreReviews.prototype.getReviews = function (app, country, page) 
{
	...
	request(url, function (error, response, body) {
		...
		if (!error && response.statusCode == 200) 
		{
					...
					{	
						...
						{		
							...
							self.emit('review', comment);
				                       ...
				                       {
					                       self.emit('error', {error: err, appId: app, country: country, status: 500});
				                       }
					}
				}
					...
					if (rel == 'last') 
					{
						...
						for (var index = 0; index < urlSplit.length; index++) 
						{
							...
							if (currentUrlPart.substring(0, 5) == 'page=') 
							{
								...
								if (page < lastPage) 
								{
									self.emit('nextPage', {'appId': app, 'country': country, 'currentPage': page, 'nextPage': page + 1, last: lastPage});
									return;
								}
								else if (page == lastPage)
								{
									self.emit('done', {'appId': app, 'country': country, status: response.statusCode});
									return;
								}
							}
						}
			...
			self.emit('error', {error:error, appId: app, country: country, status: 500});	
			return;
		}
	});
}
